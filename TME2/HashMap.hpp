...
public:
    HashMap(size_t size = 100):buckets(size),sz(0){
        /*
    ...
    bool put(const K& key, const V& value){
        size_t buckets_index = std::hash<K>()(key);
        size_t list_index = buckets_index % buckets.size();

        for (Entry & it : buckets[buckets_index]) {
            if (it.key==key) {
                it.value=value;
                return true;
            }
        ...
        sz++;
        buckets[buckets_index].emplace_front(key,value);
        return false;
